...
function ServiceCard({ service, index }: { service: (typeof services)[0]; index: number }) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const tiltX = ((e.clientY - rect.top - rect.height / 2) / (rect.height / 2)) * -4;
    const tiltY = ((e.clientX - rect.left - rect.width / 2) / (rect.width / 2)) * 4;
  ...
  return (
    <div
      className={`scroll-reveal ${service.size} group cursor-default`}
      style={{ transitionDelay: `${index * 75}ms`, perspective: "800px" }}
    >
      <div
        ref={cardRef}
        ...
        className="h-full p-[1.5px] rounded-[2rem] bg-black/[0.04] ring-1 ring-black/[0.04] transition-all duration-500 cubic-spring hover:ring-rose/20"
        style={{
          transform: isHovered
      ...
      >
        <div
          className="h-full rounded-[calc(2rem-1.5px)] bg-white p-7 md:p-9 transition-all duration-500 cubic-spring relative overflow-hidden"
          style={{
            background: isHovered
              ? `radial-gradient(circle at ${glow.x}% ${glow.y}%, #FDF6F4 0%, #FFFFFF 60%)`
              : "#FFFFFF",
            boxShadow: isHovered
              ? "0 1px 2px rgba(0,0,0,0.02), 0 24px 64px rgba(196,144,138,0.08), inset 0 1px 0 rgba(255,255,255,0.8)"
        ...
        >
          <div className="flex flex-col h-full relative z-10">
            {/* Icon with pulse */}
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center text-rose-deep mb-5 transition-all duration-500 cubic-spring ${
                isHovered
                  ? "bg-rose-light/60 scale-110 shadow-[0_0_24px_rgba(196,144,138,0.2)]"
                  : "bg-rose-light/40"
              }`}
              style={{
            ...
            >
              <div
                className={`transition-all duration-500 cubic-spring ${
                  isHovered ? "scale-110" : ""
                }`}
              >
                {service.icon}
            ...
            <h3
              className="text-lg md:text-xl font-medium tracking-tight text-text mb-3 transition-all duration-500 cubic-spring"
              style={{
            ...
            <p
              className="text-sm md:text-base text-text-muted font-light leading-relaxed flex-1 transition-all duration-500 cubic-spring"
              style={{
            ...
            <span
              className={`inline-flex items-center gap-1.5 mt-5 text-sm font-medium text-rose transition-all duration-500 cubic-spring ${
                isHovered ? "gap-2.5" : ""
              }`}
              style={{
                transform: isHovered ? "translateZ(15px)" : "translateZ(0)",
